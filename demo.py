...
class Memory:
    ...
    def store(self, state, action, prob, reward, next_state):
    ...
    def generate_batch(self, batch_size):
        ...
        indices = np.arange(length, dtype=np.int)

        batchs = [indices[i:i+batch_size] for i in starts]

        return self.states, self.actions, self.probs, self.rewards, self.next_states, batchs


class Agent:
    def __init__(self, state_dim, action_dim, gamma=0.99, lamda=0.95, clip=0.1,
                 learning_rate=0.001, batch_size=128, save_dir='./ppo_model_demo',
    ...
    def learn(self):
        states, actions, probs, rewards, next_states, batchs = self.buffer.generate_batch(self.batch_size)

        s = torch.tensor(states, dtype=torch.float).to(self.device)
        a = torch.tensor(actions, dtype=torch.float).to(self.device)
        old_logp = torch.tensor(self.buffer.probs, dtype=torch.float).to(self.device).view(-1, 1)
        r = torch.tensor(rewards, dtype=torch.float).to(self.device).view(-1, 1)
        next_s = torch.tensor(next_states, dtype=torch.float).to(self.device)

        with torch.no_grad():
            v = self.model(s)[1]
            v_ = r + self.gamma * self.model(next_s)[1]
            advantages = v_ - v  # use td-error to approximate advantage
        ...
        for _ in range(self.epochs):
            for index in batchs:
                ...
                ratio = new_logp.exp() / old_logp[index].exp()
...
def ppo_train(env, agent, n_episode=1000, update_step=2000):
    ...
    for episode in range(n_episode):
        ...
        while True:
            ...
            episode_steps += 1
            total_reward += reward
            agent.store((state, action, logp, reward, next_state))
